...
- **Frete grátis**: threshold definido em `src/lib/constants.ts` → `FREE_SHIPPING_THRESHOLD = 79` (R$ 79). `SHIPPING_COST = 19.9`. Usado em CartSheet, página de carrinho, checkout e orders. Textos no Header e homepage devem sempre refletir esse valor
- **Toaster (Sonner)**: configurado em `src/app/layout.tsx` com `position="top-left"` — mantido à esquerda para não sobrepor o CartSheet que abre pela direita
